...
(function (cdmValidationStep) {
    ...
    cdmValidationStep[cdmValidationStep["declarations"] = 1] = "declarations";
    cdmValidationStep[cdmValidationStep["references"] = 2] = "references";
    cdmValidationStep[cdmValidationStep["parameters"] = 3] = "parameters";
    cdmValidationStep[cdmValidationStep["traits"] = 4] = "traits";
    cdmValidationStep[cdmValidationStep["attributes"] = 5] = "attributes";
    cdmValidationStep[cdmValidationStep["entityReferences"] = 6] = "entityReferences";
    cdmValidationStep[cdmValidationStep["finished"] = 7] = "finished";
    cdmValidationStep[cdmValidationStep["error"] = 8] = "error";
...
class ResolvedTraitSet extends refCounted {
    ...
    mergeSetWillAlter(toMerge) {
        let traitSetResult = this;
        if (toMerge) {
            let l = toMerge.set.length;
            for (let i = 0; i < l; i++) {
                const rt = toMerge.set[i];
                if (traitSetResult.mergeWillAlter(rt))
                    return true;
            }
        }
        return false;
...
class ResolvedAttributeSet extends refCounted {
    ...
    copyNeeded(traits, appliers) {
        ...
        for (let i = 0; i < l; i++) {
            ...
            if (resAtt.resolvedTraits.mergeSetWillAlter(traits))
                return true;
            for (const resTraitApplier of appliers) {
                let applier = resTraitApplier["1"];
                let rt = resTraitApplier["0"];
...
class cdmObject {
    ...
    getResolvedTraits(set) {
        ...
        if (!this.rtsbElevated && (set == cdmTraitSet.all || set == cdmTraitSet.elevatedOnly)) {
...
class cdmObjectRef extends cdmObject {
    ...
    constructResolvedTraits(rtsb) {
        let set = rtsb.set;
        if (set == cdmTraitSet.inheritedOnly)
            return rtsb.takeReference(this.getObjectDef().getResolvedTraits(cdmTraitSet.all));
        if (set == cdmTraitSet.appliedOnly)
            set = cdmTraitSet.all;
        if (set == cdmTraitSet.elevatedOnly)
            rtsb.takeReference(this.getObjectDef().getResolvedTraits(set));
        if (this.appliedTraits) {
        ...
        if (relRts && relRts.find("does.referenceEntity")) {
            ...
            let resolveSide = (entRef) => {
                ...
                if (t && t.parameterValues && t.parameterValues.length)
                    otherAttribute = t.parameterValues.getParameterValue("identityAttribute").valueString;
                // get the attributes of the other entity and then look for the named one from the trait
                // jump over the reference to avoid resolution again
                sideOther.rasb.ownOne(entRef.getObjectDef().getResolvedAttributes().get(otherAttribute));
                return sideOther;
            };
        ...
        if (set != cdmTraitSet.appliedOnly) {
            ...
            if (set == cdmTraitSet.elevatedOnly) {
                if (this.members) {
                    let l = this.members.length;
                    for (let i = 0; i < l; i++) {
                        rtsb.mergeTraits(this.members[i].getResolvedTraits(cdmTraitSet.elevatedOnly));
                    }
                }
            }
        }
        ...
        if (set != cdmTraitSet.appliedOnly) {
            ...
            if (set == cdmTraitSet.elevatedOnly) {
                if (this.hasAttributes) {
                    ...
                    for (let i = 0; i < l; i++) {
                        rtsb.mergeTraits(this.hasAttributes[i].getResolvedTraits(cdmTraitSet.elevatedOnly));
                    }
                }
            }
        }
                ...
                if (imp.doc) {
                    if (imp.moniker && imp.moniker.length > 0) {
                        if (!this.monikeredImports.has(imp.moniker))
                            this.monikeredImports.set(imp.moniker, imp.doc);
                    }
                    else {
...
class Corpus extends Folder {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    resolveReferencesAndValidate(stage, status) {
        return new Promise(resolve => {
            let errors = 0;
            // helper
            ////////////////////////////////////////////////////////////////////////////////////////////////////
            //  declarations
            ////////////////////////////////////////////////////////////////////////////////////////////////////
            let constTypeCheck = (paramDef, aValue, userData, path, statusRpt) => {
                    ...
                    if (pValue) {
                        if (dt.isDerivedFrom("cdmObject")) {
                            ...
                            if (expectedTypes.length == 0)
                                statusRpt(cdmStatusLevel.error, `parameter '${paramDef.getName()}' has an unexpected dataType.`, path);
                            ...
                            if (expectedTypes.indexOf(foundType) == -1)
                                statusRpt(cdmStatusLevel.error, `parameter '${paramDef.getName()}' has the dataType of '${expected}' but the value '${foundDesc}' does't resolve to a known ${expected} referenece`, path);
                            else
            ...
            if (stage == cdmValidationStep.start || stage == cdmValidationStep.declarations) {
                        ...
                        switch (iObject.getObjectType()) {
                            ...
                            case cdmObjectType.entityDef:
                                if (doc.declarations.has(path)) {
                                    statusRpt(cdmStatusLevel.error, `duplicate declaration for item '${path}'`, path);
                                    return false;
                                }
                                doc.declarations.set(path, iObject);
                                iObject.corpusPath = doc.path + doc.name + '/' + path;
                                statusRpt(cdmStatusLevel.info, `declared '${path}'`, path);
                                break;
                        }
                        return false;
                    }, null, (level, msg, path) => { if (level == cdmStatusLevel.error)
                    ...
                    doc.visit(doc, "", (userData, iObject, path, statusRpt) => {
                        ...
                        if (ot == cdmObjectType.stringConstant) {
                            ...
                            if (sc.expectedType != cdmObjectType.unresolved && sc.expectedType != cdmObjectType.argumentDef) {
                                ...
                                if (!sc.resolvedReference) {
                                    statusRpt(cdmStatusLevel.error, `can't resolve reference '${sc.constantValue}' to a known object`, path);
                                }
                                else
                                    statusRpt(cdmStatusLevel.info, `    resolved '${sc.constantValue}'`, path);
                            }
                        }
                        return false;
                    }, (userData, iObject, path, statusRpt) => {
                        let ot = iObject.getObjectType();
                        if (ot == cdmObjectType.parameterDef) {
                            ...
                            let p = iObject;
                            constTypeCheck(p, null, userData, path, statusRpt);
                        }
                        return false;
                    }, (level, msg, path) => { if (level == cdmStatusLevel.error)
                        errors++; status(level, msg, path); });
                }
                ;
                if (errors > 0)
                ...
                for (let i = 0; i < l; i++) {
                    ...
                    doc.visit(doc, "", (userData, iObject, path, statusRpt) => {
                        ...
                        if (ot == cdmObjectType.traitRef) {
                            // remember the trait, arguments hit afer this
                            traitStack.push([iObject.getObjectDef(), 0]);
                        }
                        if (ot == cdmObjectType.argumentDef || (ot == cdmObjectType.stringConstant && iObject.expectedType == cdmObjectType.argumentDef)) {
                            let lastTrait = traitStack[traitStack.length - 1];
                            try {
                                let params = lastTrait["0"].getAllParameters();
                                let paramFound;
                                let aValue;
                                if (ot == cdmObjectType.argumentDef) {
                                    paramFound = params.resolveParameter(lastTrait["1"], iObject.getName());
                                    iObject.resolvedParameter = paramFound;
                                    aValue = iObject.value;
                                }
                                else {
                                    paramFound = params.resolveParameter(lastTrait["1"], null);
                                    iObject.resolvedParameter = paramFound;
                                    aValue = iObject;
                                }
                                // if parameter type is entity, then the value should be an entity or ref to one
                                // same is true of 'dataType' dataType
                                constTypeCheck(paramFound, aValue, userData, path, statusRpt);
                            }
                            catch (e) {
                                statusRpt(cdmStatusLevel.error, e.toString(), path);
                                statusRpt(cdmStatusLevel.error, `failed to resolve parameter on trait '${lastTrait["0"].getName()}'`, path);
                            }
                            lastTrait["1"] = lastTrait["1"] + 1;
                        }
                        return false;
                    }, (userData, iObject, path, statusRpt) => {
                        if (iObject.getObjectType() == cdmObjectType.traitRef) {
                            traitStack.pop();
                        }
                        return false;
                    }, (level, msg, path) => { if (level == cdmStatusLevel.error)
                ...
                let checkRequiredParamsOnResolvedTraits = (obj, path, statusRpt) => {
                    ...
                    if (rts) {
                        ...
                        for (let i = 0; i < l; i++) {
                                ...
                                for (let iParam = 0; iParam < l; iParam++) {
                                    if (rt.parameterValues.getParameter(iParam).getRequired()) {
                                        ...
                                        if (!rt.parameterValues.getValue(iParam))
                                            statusRpt(cdmStatusLevel.error, `no argument supplied for required parameter '${rt.parameterValues.getParameter(iParam).getName()}' of trait '${rt.traitName}' on '${obj.getObjectDef().getName()}'`, path);
                                        else
                                            resolved++;
                                    }
                                }
                            ...
                            if (found > 0 && found == resolved)
                                statusRpt(cdmStatusLevel.info, `found and resolved '${found}' required parameters of trait '${rt.traitName}' on '${obj.getObjectDef().getName()}'`, path);
                        }
                    }
                };
                ...
                for (let i = 0; i < l; i++) {
                    ...
                    doc.visit(doc, "", null, (userData, iObject, path, statusRpt) => {
                        ...
                        if (ot == cdmObjectType.entityDef) {
                            ...
                            checkRequiredParamsOnResolvedTraits(iObject, path, statusRpt);
                            // do the same for all attributes
                            if (iObject.getHasAttributeDefs()) {
                                iObject.getHasAttributeDefs().forEach((attDef) => {
                                    checkRequiredParamsOnResolvedTraits(attDef, path, statusRpt);
                                });
                            }
                        }
                ...
                ;
                // status(cdmStatusLevel.progress, `__paramCopy:${__paramCopy} __raCopy:${__raCopy} __rtsMergeOne:${__rtsMergeOne} __rasMergeOne:${__rasMergeOne} __rasApplyAdd:${__rasApplyAdd} __rasApplyRemove:${__rasApplyRemove}`, null);
                // __paramCopy =  __raCopy = __rtsMergeOne = __rasMergeOne = __rasApply = 0;
                if (errors > 0)
                    resolve(cdmValidationStep.error);
                else
