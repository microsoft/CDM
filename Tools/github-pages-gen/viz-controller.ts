...
function fileListToNavData(fileList : FileList) : navigatorData {
    ...
    let pathToFolder = (path : string, under : folder) : folder => {
        ...
        if (!under.folders)
            under.folders = new Array<folder>();
    ...
    for(let iFile =0; iFile < fileList.length; iFile ++) {
        ...
        if (file.name.endsWith(".cdm.json")){
            ...
            let ent : entityState = {id : `Entity${iEnt}`, createUX : makeUX, description : "", path : path, docName : file.name, name: entName,
                                        file:file, loadState : 0, folderId : f.id};
            iEnt ++;
            if (!f.entities)
                f.entities = new Array<entityState>();
            f.entities.push(ent);
        }
...
function buildNavigation(hier : folder, alternate : boolean) {
...
function clearLoadState(hier : folder) {
    if (hier.entities)
...
function collectEnts(hier : folder, idLookup) {
...
function indexResolvedEntities() {
    ...
    controller.idLookup.forEach((entState, id) => {
        if (entState.loadState == 1 && entState.entity) {
            let directives = new cdm.TraitDirectiveSet(new Set<string>(["referenceOnly", "normalized"])); // default for relational vis
            let resOpt: cdm.resolveOptions = {wrtDoc: entState.entity.declaredInDocument, directives: directives};
...
function resolveCorpus(messageType : string) {
...
function getResolutionOptions(doc: cdm.ICdmDocumentDef): cdm.resolveOptions {
        ...
        if (!data2) {
            ...
            if (data1 && entity) {
                ...
                if (atts) {
                    ...
                    // });
                    let addAtts = (rasSub:cdm.ResolvedAttributeSet, depth:number)=> {
                        let l = rasSub.set.length;
                        for (var i = 0; i < l; i++) {
                            var att = rasSub.set[i];
                            if ((att.target as cdm.ResolvedAttributeSet).set) {
                                addAtts(att.target as cdm.ResolvedAttributeSet, depth + 1);
                            }
                            else {
...
function makeParamValue(param, value) {
    ...
    let valObj = value as cdm.ICdmObject;
    if (valObj.getObjectType() == cdm.cdmObjectType.entityRef && (entDef = valObj.getObjectDef(getResolutionOptions(controller.cdmDocSelected))) && entDef.getObjectType() == cdm.cdmObjectType.constantEntityDef) {
        let entShape = entDef.getEntityShape().getObjectDef(getResolutionOptions(controller.cdmDocSelected)) as cdm.ICdmConstantEntityDef;
...
function makeParamRow(param: cdm.ICdmParameterDef, value) {
...
function makeTraitRow(rt: cdm.ResolvedTrait, alt) {
