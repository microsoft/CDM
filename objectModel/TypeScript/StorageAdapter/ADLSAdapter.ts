...
export class ADLSAdapter extends NetworkAdapter implements StorageAdapter {
    ...
    public async fetchAllFilesAsync(folderCorpusPath: string): Promise<string[]> {
        const arr: string[] = this.createFetchAllFilesUrl(this.formatCorpusPath(folderCorpusPath));
        const url: string = arr[0];
        const directory: string = arr[1];
    ...
    private createFetchAllFilesUrl(currFullPath: string): [string, string] {
        const url: string = `https://${this.hostname}/${this.fileSystem}`;
        const directory: string = `${this.subPath}/${currFullPath}`;

        return [url, directory];
    }

    private extractFilesystemAndSubPath(currFullPath: string) {
