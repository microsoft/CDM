...
describe('Persistence.PersistenceLayerTest', () => {
    ...
    //     const manifestFromOdi: CdmManifestDefinition = await corpus.fetchObjectAsync<CdmManifestDefinition>('Odi.json');
    ...
    // TODO: when ODI is ported
    // it('TestSavingInvalidModelJsonAndOdiJsonName', async (done) => {
    //     const corpus: CdmCorpusDefinition = new CdmCorpusDefinition();
    //     corpus.setEventCallback(() => { }, cdmStatusLevel.error);
    //     corpus.storage.ummount('cdm');
    //     corpus.storage.defaultNamespace = 'local';
     ...
     */
    it('TestLoadingInvalidModelJsonAndOdiJsonName', async (done) => {
        const testInputPath: string = testHelper.getInputFolderPath(testsSubpath, 'TestLoadingInvalidModelJsonAndOdiJsonName');
