...
public class LocalEntityDeclarationTest {
  ...
  public void testLoadLocalEntityNoPartition() throws IOException, InterruptedException {
    ...
    final CdmManifestDefinition cdmManifest = ManifestPersistence.fromData(
  ...
  public void testLoadLocalEntityNoPartitionAbsoluteNamespaceSet() throws IOException, InterruptedException {
    ...
    CdmManifestDefinition cdmManifest = ManifestPersistence.fromData(new ResolveContext(new CdmCorpusDefinition()), "testEntity", "testNamespace", "/", manifestContent);
    Assert.assertEquals(cdmManifest.getEntities().getCount(), 1);
    Assert.assertEquals(
        cdmManifest.getEntities().get(0).getObjectType(),
        CdmObjectType.LocalEntityDeclarationDef);
