    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      if (name.equalsIgnoreCase(this.getName())) {
        ...
        if (this.getChildFolders() != null) {
          for (final CdmFolderDefinition childFolder : this.getChildFolders()) {
            result = childFolder.fetchChildFolderFromPathAsync(remainingPath, makeFolder).join();
            if (result != null) {
              break;
            }
          }
        ...
        if (first != -1) {
          ...
          if (childPath == null) {
            LOGGER.error("Cannot find remain path '{}' from folder '{}.", remainingPath, name);
          }
          return childPath;
        }
