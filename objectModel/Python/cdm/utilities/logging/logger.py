...
def debug(tag: str, ctx: 'CdmCorpusContext', message: str, path: Optional[str] = None) -> None:
    _log(CdmStatusLevel.PROGRESS, ctx, _format_message(tag, message, path), default_logger.debug)


def info(tag: str, ctx: 'CdmCorpusContext', message: str, path: Optional[str] = None) -> None:
    _log(CdmStatusLevel.INFO, ctx, _format_message(tag, message, path), default_logger.info)


def warning(tag: str, ctx: 'CdmCorpusContext', message: str, path: Optional[str] = None) -> None:
    _log(CdmStatusLevel.WARNING, ctx, _format_message(tag, message, path), default_logger.warning)


def error(tag: str, ctx: 'CdmCorpusContext', message: str, path: Optional[str] = None) -> None:
    _log(CdmStatusLevel.ERROR, ctx, _format_message(tag, message, path), default_logger.error)


def _log(level: 'CdmStatusLevel', ctx: 'CdmCorpusContext', message: str, default_status_event: Callable) -> None:
    if level >= ctx.report_at_level:
        if ctx and ctx.status_event:
            ctx.status_event(level, message)
        else:
            default_status_event(message)
